...
MES 2.1.3 では UI の前面維持ロジックを強化し、外部オーバーレイやページ UI が後から追加されても MES ボタンとパネルが最前面に残るようにしました。

## 2.1.2

MES 2.1.2 では、iframe 内にダッシュボードボタンが重複して表示されないようにしました。保存済みのブロックルールと DOM 監視は iframe 内でも動作しますが、表示されるボタンとパネルは最上位ページにだけ作成されます。
