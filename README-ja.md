...
- ルール復元で Picky のサイト別ルール JSON をサポート
...
MES の `設定` > `ルールを復元 (JSON)` は Picky のサイト別ルール JSON も受け付けます。Picky からルール JSON をエクスポートまたはコピーし、`.json` ファイルとして保存してから MES の復元機能で選択してください。
