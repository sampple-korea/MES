...
MES 2.1.3은 외부 오버레이나 다른 페이지 UI가 나중에 추가되어도 MES 버튼과 패널이 최상단에 유지되도록 UI 전면 유지 로직을 강화한 패치입니다.

## 2.1.2

MES 2.1.2는 iframe 안에서 대시보드 버튼이 중복으로 생기지 않도록 정리한 패치입니다. iframe 안에서는 저장된 차단 규칙과 DOM 감시는 계속 동작하지만, 버튼/패널 UI는 최상위 페이지에만 표시됩니다.

## 2.1.1

MES 2.1.1은 설정에서 UI 언어를 한국어, 영어, 중국어, 일본어로 바꿀 수 있는 다국어 마이너 업데이트입니다. 기본 언어는 한국어이며, 설치/업데이트는 계속 GitHub raw의 `MES.min.js`를 사용합니다.

자세한 변경점은 [RELEASE_NOTES_2.1.1.md](RELEASE_NOTES_2.1.1.md)를 참고하세요. 2.1.0의 선택자/성능 개선은 [RELEASE_NOTES_2.1.0.md](RELEASE_NOTES_2.1.0.md)에 정리되어 있습니다.
